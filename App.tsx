...
import { PilgrimageCardBackground } from './components/PilgrimageCardBackground';
...
const App: React.FC = () => {
  ...
  const roles: Role[] = [
    ...
    { cn: "批评家", en: "CRITIC" }, { cn: "行为艺术家", en: "PERFORMANCE ARTIST" }, { cn: "观众", en: "VIEWER" }
  ];
  ...
  return (
    <div className={`min-h-screen text-black font-sans selection:bg-black selection:text-white overflow-x-hidden relative ${isModalOpen ? 'h-screen overflow-hidden' : ''}`}>
      ...
      <div className="fixed top-0 left-0 w-full bg-black text-white z-50 border-b-2 border-white p-1 overflow-hidden flex whitespace-nowrap">
        <div className="animate-marquee flex-shrink-0 font-mono text-[10px] md:text-xs tracking-[0.2em] uppercase pr-8">
          WARNING: REALITY COMPROMISED /// 警告：现实已受损 /// WELCOME TO THE DESERT OF THE REAL /// 欢迎来到实在界这个大荒原 /// NO IMAGES ALLOWED /// 纯粹文本主义 /// WARNING: REALITY COMPROMISED /// 警告：现实已受损 /// WELCOME TO THE DESERT OF THE REAL /// 欢迎来到实在界这个大荒原 /// NO IMAGES ALLOWED /// 纯粹文本主义 ///
        </div>
        <div className="animate-marquee flex-shrink-0 font-mono text-[10px] md:text-xs tracking-[0.2em] uppercase pr-8" aria-hidden="true">
          WARNING: REALITY COMPROMISED /// 警告：现实已受损 /// WELCOME TO THE DESERT OF THE REAL /// 欢迎来到实在界这个大荒原 /// NO IMAGES ALLOWED /// 纯粹文本主义 /// WARNING: REALITY COMPROMISED /// 警告：现实已受损 /// WELCOME TO THE DESERT OF THE REAL /// 欢迎来到实在界这个大荒原 /// NO IMAGES ALLOWED /// 纯粹文本主义 ///
        </div>
      </div>
      ...
      <header className="relative min-h-screen border-b-8 border-black flex flex-col pt-8 md:pt-12 bg-white">
        ...
        {currentView === 'exhibition' ? (
          <div className="flex-grow flex flex-col items-center justify-center p-8 relative z-10 bg-black text-white">
            <button
              onClick={() => setCurrentView('home')}
              className="absolute top-8 left-8 border-2 border-white px-4 py-2 font-mono text-xs uppercase hover:bg-white hover:text-black transition-colors"
            >
              &lt; Return_to_Base
            </button>
            <div className="text-center">
              <h1 className="text-6xl md:text-9xl font-black mb-4 tracking-tighter">此山行</h1>
              <p className="font-mono text-xl opacity-70 mb-8">JOURNEY TO THIS MOUNTAIN</p>
              <div className="border border-white/30 p-4 font-mono text-sm max-w-md mx-auto">
                STATUS: EXHIBITION_LOADED<br />
                TARGET: DA_LIANG_SHAN_CHILDREN<br />
                MODE: ONLINE_VIEWING
              </div>
            </div>
          </div>
          ...
          <div className="flex-grow flex flex-col justify-between relative z-10 px-4 md:px-8 pb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 my-8 font-mono uppercase">
              {/* FEATURED EVENT: 此山行 */}
              <div
                onClick={() => window.location.href = '/pilgrimage/index.html'}
                className="cursor-pointer border-4 border-black p-6 flex flex-col bg-white text-black transition-all duration-300 group relative overflow-hidden min-h-[200px] md:min-h-[180px] md:col-span-2 lg:col-span-2"
              >
                <PilgrimageCardBackground className="opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="absolute top-0 right-0 bg-black text-white px-2 py-1 text-[10px] font-black uppercase z-20 transition-colors">OFFICIAL_EVENT</div>

                <div className="flex justify-between items-start z-10 w-full mb-4 relative">
                  <div className="text-[10px] font-black border-2 border-current px-1.5 py-0.5 tracking-tight group-hover:border-white group-hover:text-white group-hover:drop-shadow-[0_2px_3px_rgba(0,0,0,0.6)] transition-all">限时活动</div>
                  <ArrowUpRight size={24} className="group-hover:rotate-45 transition-transform group-hover:text-white group-hover:drop-shadow-[0_2px_3px_rgba(0,0,0,0.6)]" />
                </div>

                <div className="z-10 flex-grow flex flex-col justify-center relative">
                  <h3 className="text-4xl md:text-6xl font-black leading-[0.9] tracking-tighter uppercase mb-2 relative inline-block self-start group-hover:text-white group-hover:drop-shadow-[0_5px_8px_rgba(0,0,0,0.7)] transition-colors">
                    此山行
                    <span className="absolute -top-1 left-full ml-4 text-xs font-mono font-bold tracking-widest animate-[pulse_1s_ease-in-out_infinite] whitespace-nowrap text-black group-hover:text-white group-hover:drop-shadow-[0_2px_3px_rgba(0,0,0,0.6)] transition-colors">NEW</span>
                  </h3>
                  <p className="font-sans font-bold text-sm md:text-lg leading-tight opacity-70 group-hover:opacity-100 group-hover:text-white group-hover:drop-shadow-[0_2px_3px_rgba(0,0,0,0.6)] transition-all">
                    给大凉山孩子们的线上艺术展
                  </p>
                </div>

                <div className="mt-4 pt-4 flex justify-between items-center opacity-60 group-hover:opacity-100 group-hover:text-white group-hover:drop-shadow-[0_2px_3px_rgba(0,0,0,0.6)] relative z-10 transition-all">
                  <span className="text-[10px]">ONLINE_GALLERY</span>
                  <span className="text-[10px]">NOW_OPEN</span>
                </div>
              </div>
                  ...
                  <div className="flex items-end gap-[3px] h-6 mt-4 opacity-70">
                    {[...Array(8)].map((_, i) => (
                      <div key={i} className="w-1.5 bg-current origin-bottom" style={{
                        animation: `equalizer ${0.4 + Math.random() * 0.4}s ease-in-out ${i * 0.1}s infinite alternate`,
                      }}></div>
                    ))}
              ...
              <div className="border-4 border-black min-h-[260px] md:min-h-[300px] md:col-span-2 lg:col-span-4 relative overflow-hidden bg-white isolate group">
                ...
                <div className="absolute inset-0 w-full h-full overflow-hidden">
                  <div className="animate-credits w-full">
                    {[0, 1].map((iteration) => (
                      <div key={iteration} className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-2 p-6 pt-8 pb-6 border-b-2 border-dashed border-gray-300">
                        {roles.map((role, i) => (
              ...
              {ideas.map((idea, index) => (
                <div key={index} className={`relative p-8 md:p-12 flex flex-col justify-between min-h-[300px] md:min-h-[480px] transition-all duration-500 group overflow-hidden ${idea.isSpecial ? 'bg-black text-white' : 'bg-white hover:bg-black hover:text-white text-black'}`}>
                  ...
                  <div className="flex-grow relative z-10">
                    <h4 className="text-3xl md:text-6xl font-black mb-4 md:mb-6 leading-[0.9] tracking-tight font-sans whitespace-pre-line group-hover:translate-x-2 transition-transform duration-500 break-all">{idea.title}</h4>
